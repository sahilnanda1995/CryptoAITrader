...
price_datafile_arr = ["../bit_2013to2019_1dayCandle.csv", "../3yr4mon1hr_bit.csv"]
trainSizeArr = [2128, 30004]
logsFileNameArr = ['1day_seq_pred2', '1hr_seq_pred'] 

for x in range(len(price_datafile_arr) - 1):
	input_file_3yr = price_datafile_arr[x]
	...
	def create_dataset(dataset, vol_dataset, look_back=1):
		...
		for i in range(len(dataset)-look_back-1-forecastCandle):
			...
			dataX.append(c)
			# print('dataX', dataX)
			y = dataset[i + look_back + forecastCandle:i + look_back + forecastCandle+2, 1:3]
			# print('y', y)
			y = y.reshape(-1, 1)
			# print('y', y)
	...
	trainYArr = trainYArr.reshape(-1, 4)
	...
	df = pd.DataFrame(data={"timeStamp": np.around(list(train_timeStamp[-1].reshape(-1)), decimals=2), "high_prediction1": np.around(list(testPredict[0:1].reshape(-1)), decimals=2), "low_prediction1": np.around(list(testPredict[1:2].reshape(-1)), decimals=2), "high_prediction2": np.around(list(testPredict[2:3].reshape(-1)), decimals=2), "low_prediction2": np.around(list(testPredict[3:4].reshape(-1)), decimals=2), "test_price": np.around(list(arr2.reshape(-1)), decimals=2), "volume": np.around(list(train_volume_dataset.reshape(-1)), decimals=2), "entry_test_price": np.around(list(testXArr[len(testXArr)-1].reshape(-1)), decimals=2), "exit_high_price1": np.around(list(testYArr[0].reshape(-1)), decimals=2), "exit_low_price1": np.around(list(testYArr[1].reshape(-1)), decimals=2), "exit_high_price2": np.around(list(testYArr[2].reshape(-1)), decimals=2), "exit_low_price2": np.around(list(testYArr[3].reshape(-1)), decimals=2)})
	file_name = logsFileNameArr[x] 
	df.to_csv(file_name, sep=';', index=None)

	step = 1

	for i in range(trainSizeArr[x]+step, len(dataset)-1, step):
		train_size = i
		dataset_len = len(dataset) 
		# print(len(dataset))
		test_size = len(dataset) - train_size + look_back
