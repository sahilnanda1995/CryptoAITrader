...
df = pd.DataFrame(data={"timeStamp": np.around(list(train_timeStamp[-1].reshape(-1)), decimals=2), "high_prediction1": np.around(list(testPredict[0:1].reshape(-1)), decimals=2), "low_prediction1": np.around(list(testPredict[1:2].reshape(-1)), decimals=2), "volume": np.around(list(train_volume_dataset.reshape(-1)), decimals=2), "entry_test_price": np.around(list(testXArr[len(testXArr)-1].reshape(-1)), decimals=2), "exit_high_price1": np.around(list(testYArr[0].reshape(-1)), decimals=2), "exit_low_price1": np.around(list(testYArr[1].reshape(-1)), decimals=2)})
file_name = "1day_accuracy2.csv" 
df.to_csv(file_name, sep=';', index=None)

step = 1

for i in range(2128+step, len(dataset)-1, step):
	train_size = i
	dataset_len = len(dataset) 
	# print(len(dataset))
	test_size = len(dataset) - train_size + look_back
