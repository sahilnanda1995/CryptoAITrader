...
df = pd.DataFrame(data={"timeStamp": np.around(list(train_timeStamp[-1].reshape(-1)), decimals=2), "high_prediction": np.around(list(testPredict[0:1].reshape(-1)), decimals=2), "low_prediction": np.around(list(testPredict[1:2].reshape(-1)), decimals=2), "close_prediction": np.around(list(testPredict[2:3].reshape(-1)), decimals=2), "test_price": np.around(list(arr2.reshape(-1)), decimals=2), "volume": np.around(list(train_volume_dataset.reshape(-1)), decimals=2), "entry_test_price": np.around(list(trainY.reshape(-1)), decimals=2), "exit_high_price": np.around(list(test[-3:-2].reshape(-1)), decimals=2), "exit_low_price": np.around(list(test[-2:-1].reshape(-1)), decimals=2)})
file_name = "HLC_pred_volume_metcalfe_tx_HCLO_lookBack_730_model57_jan_to_may.csv" 
