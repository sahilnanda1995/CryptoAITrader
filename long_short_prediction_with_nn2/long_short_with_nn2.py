...
import time
import nn2_model_long_short_predictor
...
def create_dataset(dataset, vol_dataset, diff_dataset, look_back=1):
	dataX, dataY = [], []
	for i in range(len(dataset)-look_back-1-forecastCandle):
		...
		diffData = diff_dataset[i:(i+look_back)]
		# print('b', b[-10:])
		# a = a.reshape(-1, 1)
		...
		dataY.append(dataset[i + look_back + forecastCandle, 1:4])
		# print('dataY', dataY)
...
print('df4_difficulty', df4_difficulty[0:10])
...
trainX, trainY = create_dataset(train, train_volume_dataset, train_diff_dataset, look_back)
testX, testY = create_dataset(test, test_volume_dataset, test_diff_dataset, look_back)

print('trainX all inputs', trainX[-1])



trainXArr_mltx = []
for val in trainX[len(trainX)-1]:
	trainXArr_mltx.append(val[5])

trainXArr_mltx = np.array(trainXArr_mltx)
trainXArr_mltx = trainXArr_mltx[-10:]
trainXArr_mltx = trainXArr_mltx.reshape(-1,1)
print(trainXArr_mltx)
trainXArr_mltx = scaler_diff.inverse_transform(trainXArr_mltx)

print('trainXArr_mltx', trainXArr_mltx)
...
trainYArr = trainYArr.reshape(-1, 3)
...
for val in testX[len(testX)-1]:
	testXArr_mltx.append(val[5])


testXArr_mltx = np.array(testXArr_mltx)
testXArr_mltx = testXArr_mltx[-10:]
testXArr_mltx = testXArr_mltx.reshape(-1,1)
print(testXArr_mltx)
testXArr_mltx = scaler_diff.inverse_transform(testXArr_mltx)

print('testXArr_mltx', testXArr_mltx)


testXArr_vol = []
for val in testX[len(testX)-1]:
	testXArr_vol.append(val[4])


testXArr_vol = np.array(testXArr_vol)
testXArr_vol = testXArr_vol[-10:]
testXArr_vol = testXArr_vol.reshape(-1,1)
print(testXArr_vol)
testXArr_vol = scaler_vol.inverse_transform(testXArr_vol)

print('testXArr_vol', testXArr_vol)
...
window_size, input_shape, dropout_value, activation_function, loss_function, optimizer = look_back, 6, 0.2, 'linear', 'mse', 'adam'
...
print(trainY[0][0], testPredict[0][0], testPredict[1][0], testXArr_vol[len(testXArr_vol)-1][0])

# callTakingProb = nn2_for_1dayCandle.predict_value(trainY, testPredict, train_volume_dataset)
long_or_short = nn2_model_long_short_predictor.predict_action(trainY[0][0], testPredict[0][0], testPredict[1][0], testXArr_vol[len(testXArr_vol)-1][0])

# export prediction and actual prices
df = pd.DataFrame(data={"timeStamp": np.around(list(train_timeStamp[-1].reshape(-1)), decimals=2), "high_prediction": np.around(list(testPredict[0:1].reshape(-1)), decimals=2), "low_prediction": np.around(list(testPredict[1:2].reshape(-1)), decimals=2), "close_prediction": np.around(list(testPredict[2:3].reshape(-1)), decimals=2), "test_price": np.around(list(arr2.reshape(-1)), decimals=2), "volume": np.around(list(testXArr_vol[-1:].reshape(-1)), decimals=2), "ml_tx": np.around(list(testXArr_mltx[-1:].reshape(-1)), decimals=2), "entry_test_price": np.around(list(trainY.reshape(-1)), decimals=2), "exit_high_price": np.around(list(test[-3:-2].reshape(-1)), decimals=2), "exit_low_price": np.around(list(test[-2:-1].reshape(-1)), decimals=2), "long_or_short": np.around(list(long_or_short[0].reshape(-1)), decimals=2)})
file_name = "long_or_short_prediction.csv" 
df.to_csv(file_name, sep=';', index=None)

step = 1
# trades_count = 1
for i in range(2098+step, len(dataset)-1, step):
	train_size = i
	dataset_len = len(dataset) 
	# print(len(dataset))
	test_size = len(dataset) - train_size + look_back
