...
difficulty_input = "long_short_data.csv"
	...
	for i in range(len(dataset)-look_back-1-forecastCandle):
		...
		for j in range(len(b)):
			for k in range(len(a[j])):
				c.append(a[j][k])
			c.append(b[j])
			for l in range(len(diffData[j])):
				c.append(diffData[j][l])
		c = np.array(c)
		c = c.reshape(-1, 1)
		c = c.reshape(-1, 7)
		# print('c', c)
...
df4_difficulty = read_csv(difficulty_input, header=None, index_col=None, delimiter=',', usecols=[4, 11])
...
df4_difficulty = df4_difficulty.reshape(-1, 2)
...
trainX = np.reshape(trainX, (trainX.shape[0], 7, trainX.shape[1]))
testX = np.reshape(testX, (testX.shape[0], 7, testX.shape[1]))


# create and fit the LSTM network, optimizer=adam, 25 neurons, dropout 0.1
model = Sequential()
model.add(LSTM(25, input_shape=(7, look_back)))
...
file_name = "pred_1day_nn2_with_volume_both_longs_and_shorts_lookBack_730.csv" 
df.to_csv(file_name, sep=';', index=None)

step = 1
# trades_count = 1
for i in range(497+step, len(dataset)-10, step):
	train_size = i
	dataset_len = len(dataset) 
	# print(len(dataset))
	test_size = len(dataset) - train_size + look_back
