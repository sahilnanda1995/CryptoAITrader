...
difficulty_input = "funding_rateData.csv"
...
df4_difficulty = read_csv(difficulty_input, header=None, index_col=None, delimiter=';', usecols=[4])
...
file_name = "pred_1day_nn2_with_volume_fundingRate_lookBack_730.csv" 
df.to_csv(file_name, sep=';', index=None)

step = 1
# trades_count = 1
for i in range(960+step, len(dataset)-10, step):
	train_size = i
	dataset_len = len(dataset) 
	# print(len(dataset))
	test_size = len(dataset) - train_size + look_back
